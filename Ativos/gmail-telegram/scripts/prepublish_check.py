...
SENSITIVE_PATH = re.compile(
    r"(^|/)(\.env($|\.)|config\.py$|credentials?|secrets?|.*token.*\.json$|"
...
ALLOWED_EXAMPLES = {".env.example", "config.example.py"}
...
def scan_current(findings: set[str]) -> None:
    ...
    for relative in tracked_files():
        ...
        try:
            scan_blob(f"possivel segredo: {relative}", (root / relative).read_bytes(), findings)
        except OSError:
